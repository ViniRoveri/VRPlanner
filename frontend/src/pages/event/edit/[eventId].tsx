...
import { formStyles, loadingEvent } from "@/common/globalConstants"
import { Ctx } from "@/common/types"
import DefaultHead from "@/components/DefaultHead"
import EditEventForm from "@/components/EventsForms/EditEventForm"
...
export default function EventEditPage(){
   ...
   return (
      <>
      <DefaultHead title="Edit Event | VR Planner"/>

      <GoBackArrow/>

      <main className={stylesContainer}>
         <h2 className={stylesTitle}>Edit event:</h2>

         <EventCard event={selectedEvent} monthDisplay={monthDisplay} scope={'edit'}/>

         <EditEventForm selectedEvent={selectedEvent}/>

         <p className={formStyles.redirectP}>if you want to delete this event click <Link href={`/event/delete/${eventId}`} className={formStyles.redirectLink}>here</Link></p>
      </main>
      </>
   )
}